...
from modules.mixers.qlinear import QlinearMixer
import torch as th
...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            ...
            elif args.mixer == 'qlinear':
                self.mixer = QlinearMixer(args)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            ...
            elif self.mixer.name == 'qlinear':
                chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], actions)
                target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], target_next_actions)
            else:
